...
- 엔티티 생명주기 설명
  - 엔티티 생명주기 종류
    ...
    4. 삭제 
     ...
     ##### 비영속에서 영속으로 전환
       - 비영속과 영속성의 특징은 위에서 이야기 했지만 한줄로 표현하자면 아래와 같다.
         - 비영속 : 엔티티가 영속성 컨텍스트에 아직 담기지 않은 상태
         - 영속 : 엔티티가 영속성 컨텍스트에 담겨 관리 받고 있는 상태<br/>
           &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;  단, DB에 저장은 되지 않은 상태
 
       - 비영속에서 영속으로 전환되는 흐름은 아래와 같으며 이해가 되지 않는다면 아래의 시각화를 참조하시오.
         1. entity 매니저에 엔티티에 해당 데이터를 담아서 <b>" .persist(); "</b>를 선언하여 비영속을 영속상태로 변환
            단, 여기서 완벽한 영속 상태는 아니며 DB에 접근(저장) 되지 않았기 때문이다.
            
         2. 1번 작업으로 인해서 모아둔 퀄문을 <b>" .flush(); "</b> 실행할 겨우 DB에 접속하여 반영을 시킨다.
